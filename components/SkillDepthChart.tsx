  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-slate-900">
      ...
      <div className="overflow-x-auto">
        <div className="flex min-w-[480px] items-end gap-3 pb-2">
          {groups.map((group, i) => {
            ...
            return (
              <div key={group.name} className="flex flex-1 flex-col items-center gap-2">
                ...
                >
                  <motion.div
                    className={`relative w-full rounded-t-lg ${color.bar}`}
                    initial={{ height: 0 }}
                    animate={{ height: targetHeight }}
                    transition={{ duration: 0.7, delay: i * 0.08, ease: "easeOut" }}
                  >
                    {/* Years label inside/above the bar */}
                    <span className="absolute -top-6 left-0 right-0 text-center text-xs font-bold text-zinc-600 dark:text-zinc-300">
                      {group.yearsExp}yr
                    </span>
                  </motion.div>
                </div>
                ...
                {/* Category name */}
                <p className="text-center text-xs font-semibold text-zinc-700 dark:text-zinc-300 leading-tight">
                  {group.name}
                </p>

                {/* Top 2 skills as tiny pills */}
